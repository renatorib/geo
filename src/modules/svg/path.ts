// Ported from https://github.com/svg/svgo/blob/main/lib/path.js
...
const argsCountPerCommand: Record<PathDataCommand, number> = {
  M: 2,
  m: 2,
  Z: 0,
  z: 0,
...
const isCommand = (cmd: string): cmd is PathDataCommand => {
  return cmd in argsCountPerCommand;
};
...
export const parsePathData = (string: string) => {
  const pathData: Array<PathDataItem> = [];
 ...
 */
const stringifyArgs = (command: string, args: number[], precision?: number, disableSpaceAfterFlags?: boolean) => {
  let result = "";
  let prev = "";
...
export const stringifyPathData = ({ pathData, precision, disableSpaceAfterFlags }: StringifyPathDataOptions) => {
  ...
  for (let i = 0; i < pathData.length; i += 1) {
    ...
    if (i === 0) {
      ...
      const last: PathDataItem = combined[combined.length - 1];
      ...
      if (
        (last.command === command && last.command !== "M" && last.command !== "m") ||
